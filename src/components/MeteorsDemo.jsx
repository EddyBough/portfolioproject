  ...
  return (
    <div className="w-full relative max-w-xs">
      ...
      <div className="relative shadow-xl bg-card1 px-4 py-8 h-full overflow-hidden rounded-2xl flex flex-col items-center">
        {imageSrc && (
          <img src={imageSrc} alt={title} className="mb-4 w-20 h-17 object-cover rounded-lg" />
        )}
        <h1 className="font-bold text-l text-white mb-4 relative z-50 text-center">{title}</h1>
        <p className="font-normal text-base text-slate-500 mb-4 relative z-50 text-white text-center">{description}</p>
        <Meteors number={20} />
      </div>
    </div>
  );
