      ...
      <div className="mt-1">
        ...
        <div className="hidden md:grid m-28 mt-10 justify-items-center grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {skillsData.map((skill) => (
            <div key={skill.name} className='flex flex-col items-center'>
              <img src={skill.imgSrc} alt={`${skill.name} picture`} className={`mt-2 h-20`} />
        ...
        {showSkills && (
          <div className="md:hidden lg:hidden grid justify-items-center place-content-center text-center grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 m-1 mt-20">
            {skillsData.map((skill) => (
              <div key={skill.name} className={`flex flex-col place-content-center items-center w-1/2 h-1/2 ${skill.name === 'Sass' || skill.name === 'Tailwind' ? 'mt-4' : ''}`}>
              <img src={skill.imgSrc} alt={`${skill.name} picture`} className={`mt-2 h-20 sm:h-16`} />
              <p className='mt-2'>{skill.name}</p>
            </div>
